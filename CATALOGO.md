...
| `portfolio/` | Funcional como demo | Mantener consistencia editorial y revisar posicionamiento de `p02`, `p03`, `p07` |
...
| `p05-rag-documentacion-interna` | Funcional y presentable | Evolucionarlo solo si se abre a mas dominios |
| `p06-rag-contratos-legales` | Funcional y presentable | Mantener clara la separacion respecto al producto legal serio |
| `p07-chatbot-atencion-cliente` | Simple y estable | Mejor onboarding y limites |
| `p08-rag-normativa-comercio` | Funcional y presentable | Reforzar solo si se incorporan fuentes oficiales versionadas |
| `p09-evaluador-ideas-negocio` | Funcional y presentable | Mejorar solo si se quiere mas trazabilidad de fuentes |
| `p10-dashboard-lenguaje-natural` | Funcional y presentable | Endurecer sandbox si pasa a uso multiusuario |
...
1. Mantener fuera del repo artefactos locales: `.pytest-tmp/`, `.pytest_cache/`, `data/`, `exports/`, `.venv/`.
2. Documentar despliegue y variables de entorno por producto si se van a publicar.
3. Confirmar externamente la rotacion de secretos documentada en `SECURITY_ROTATION.md`.
